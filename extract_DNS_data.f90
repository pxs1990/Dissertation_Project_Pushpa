...
!isLocExtrOmg
        ...
        !Bljm=(a-transpose(a))/2
        !Matljm=matmul(Aljm,Aljm)+matmul(Bljm,Bljm)
        Matljm=Aljm
...
! end of the hessain matrix computation
   ...
   ! goto 1234
    ...
    open(18, file='RortexZDZpoints.txr')
    ...
    do k = 1, kmax_local
      do j = 1, jmax_local
        ...
        !omgGradXRortex(i,j,k)=omgLoc_x(i,j,k)*rortex_x(i,j,k) +omgLoc_y(i,j,k)*rortex_y(i,j,k) &
         !       +omgLoc_z(i,j,k)*rortex_z(i,j,k)
          !ljmtmp2=rortex_mag_x(i,j,k)*rortex_x(i,j,k) +rortex_mag_y(i,j,k)*rortex_y(i,j,k) &
         !       +rortex_mag_z(i,j,k)*rortex_z(i,j,k)
       ...
       ! if(rortex_mag(i,j,k)<1e-12) then
        !    isLocExtrOmg(i,j,k)=0.0
        !endif
        ...
        if(isLocExtrOmg(i,j,k).eq. 1.0) then
         ...
         !normljm=sqrt(rortex_mag_x(i,j,k)**2+rortex_mag_y(i,j,k)**2+rortex_mag_z(i,j,k)**2 )
         ...
         ! if (rortex_mag(i,j,k)<1e-8) then
         !rortexEigr(i,j,k)  =0.0

!         if(rortex_mag(i,j,k) > 1.0e-5 .and. ljmtmp2> 1.0e-5 .and. ljmtmp<1.0e-8 ) then
         !     rortexEigr(i,j,k)=1.0
         ! else
              !rortexEigr(i,j,k)= abs(ljmtmp)/(normljm)/(rortex_mag(i,j,k))
          !    rortexEigr(i,j,k)=0.0
 !        end if
          
        !  if (normljm<=1.0e-12 .or. rortex_mag(i,j,k)<= 1.0e-12) then
         !     rortexEigr(i,j,k)=0.0
          !endif
       ...
       ! if(rortexEigr(i,j,k)>0.95) then
        !    write(18,"(4(f21.12))") x(i+istart-1, j+jstart-1, k+kstart-1),  &
         !       y(i+istart-1, j+jstart-1, k+kstart-1),  &
          !      z(i+istart-1, j+jstart-1, k+kstart-1), rortexEigr(i,j,k)
        !end if

         ! if() then

        !  normljm2=sqrt(vorticity_x(i,j,k)**2+vorticity_y(i,j,k)**2+vorticity_z(i,j,k)**2 )
     !     omgGradXRortex(i,j,k)=vorticity_x(i,j,k)*rortex_x(i,j,k) +vorticity_y(i,j,k)*rortex_y(i,j,k) &
      !          +vorticity_z(i,j,k)*rortex_z(i,j,k)

        !  omgGradXRortex(i,j,k)=vorticity_x(i,j,k)*rortex_mag_x(i,j,k) +vorticity_y(i,j,k)*rortex_mag_y(i,j,k) &
        !        +vorticity_z(i,j,k)*rortex_mag_z(i,j,k)
         omgGradXRortex(i,j,k)=1.0
         if(localOmega(i,j,k)<0.51) then
             omgGradXRortex(i,j,k)=0.0
            ! rortexEigr(i,j,k)=0.0
         !else
          !   omgGradXRortex(i,j,k)=abs(omgGradXRortex(i,j,k)/(normljm2*normljm))
             !omgGradXRortex(i,j,k)=abs(omgGradXRortex(i,j,k)/(rortex_mag(i,j,k)*normljm))
         endif

        ! if(omgGradXRortex(i,j,k)< 0.6) then
         !    omgGradXRortex(i,j,k)=0.0
        ! end if
    ...
    outputfilename1 = trim(outputfilename)//'_'//chars//".xyz"
    outputfilename2 = trim(outputfilename)//'_'//chars//".fun"
